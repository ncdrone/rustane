...
#[test]
fn leaky_relu_sq_six_layer_loss_decreases_on_fixed_sample() {
    let cfg = ModelConfig::gpt_karpathy_leaky_relu_sq();
    ...
    assert!(
        ...
        "LeakyReLU^2 loss should decrease on a fixed sample: first={first_loss:.4}, last={last_loss:.4}"
    );
}

...
#[test]
fn pg_scaffold_loss_decreases_on_fixed_sample() {
    let cfg = ModelConfig::gpt_karpathy_pg();
    ...
    assert!(
        ...
        "PG scaffold loss should decrease on a fixed sample: first={first_loss:.4}, last={last_loss:.4}"
    );
}

...
pub fn compile_mla_kernels(
    ...
    let q_a_out = TensorData::new(Shape { batch: 1, channels: q_lora_rank, height: 1, width: seq });
    ...
    let q_b_out = TensorData::new(Shape { batch: 1, channels: q_total, height: 1, width: seq });
    ...
    let kv_a_out = TensorData::new(Shape { batch: 1, channels: kv_out_dim, height: 1, width: seq });
    ...
    let o_out = TensorData::new(Shape { batch: 1, channels: hidden, height: 1, width: seq });
...
pub fn ane_projection(
    ...
    // Stage into IOSurface (channel-interleaved layout)
    {
        ...
        // Zero the buffer first (pad region must be zero)
        for v in buf.iter_mut() { *v = 0.0; }

        // Write activation: buf[c * sp + 0] = activation[c]
        for c in 0..ic {
            buf[c * sp + 0] = activation[c];
        }
        // Write weights: buf[c * sp + seq + j] = weights[j * ic + c]
        // weights is [oc, ic] row-major → weights[j][c] = weights[j * ic + c]
        // ANE Conv1x1 expects weight in spatial dim as column-major of the transpose
        for c in 0..ic {
            for j in 0..oc {
                buf[c * sp + seq + j] = weights[j * ic + c];
            }
        }
    }
    ...
    // Read output: [1, OC, 1, 1] channel-interleaved
